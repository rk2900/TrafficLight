...
public class TrafficMonitor{
	...
	public TrafficMonitor(String data, 
		...
		for (int inter:TrafficLightMap.getAll()){
			...
			for (int src:roads){
				...
				int left_through=(status.getStatus(inter, src, 1)==1)?2:0;
				int straight_through=(status.getStatus(inter, src, 2)==1)?16:0;
				int right_through=(status.getStatus(inter, src, 3)==1)?2:0;
				
				int left_cnt=(int)Math.ceil(flow*initTurnRate[0]);
				int straight_cnt=(int)Math.ceil(flow*initTurnRate[2]);
				int right_cnt=(int)Math.ceil(flow*initTurnRate[1]);
	...
	public String getVisual(TrafficLightStatus status){
		StringBuilder sb=new StringBuilder();
		List<String> visual=FileOps.LoadFilebyLine("visualmap");
